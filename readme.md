...
| `esbuild` | `^0.28.1` | Transitive via `vite@^7.1.0` (devDependency). Vite 7.x declares `esbuild@^0.25.0` which caps resolution at `<0.26.0`, preventing the lockfile from picking up the patched 0.28.1. Global resolution forces the patched version. Only one major of esbuild exists in the tree so no multi-major risk. | Remove when `vite` releases a version whose range declares `esbuild@>=0.28.1`. Track upstream: https://github.com/vitejs/vite/releases. Verify with `3pp-util trace-vuln esbuild`. Reviewer-local: `yarn why esbuild`. | GHSA-gv7w-rqvm-qjhr |
